...
public class UserBOImp implements UserBO {
	...
	public List<ResQueryBadges> getUserBadges(String username, String campusSession) {
		List<ResQueryBadges> listOfBadges = new ArrayList<ResQueryBadges>();
		try {
			if (sBo.validateSession(campusSession) != null) {
				listOfBadges = uDao.getUserBadges(username);
			}
		} catch (Exception e) {
			e.printStackTrace();
			return null;
	...
	public List<ResQueryAbilities> getUserAbilities(String username, String campusSession) {
		List<ResQueryAbilities> listOfAbilities = new ArrayList<ResQueryAbilities>();
		try {
			if (sBo.validateSession(campusSession) != null) {
				listOfAbilities = uDao.getUserAbilities(username);
			}
		} catch (Exception e) {
			e.printStackTrace();
			return null;
	...
	public List<ResQueryProficiencies> getUserProficiencies(String username, String idAbility, String campusSession) {
		List<ResQueryProficiencies> listOfProficiencies = new ArrayList<ResQueryProficiencies>();
		try {
			if (sBo.validateSession(campusSession) != null) {
				int abilityIdent = Integer.parseInt(idAbility);
				listOfProficiencies = uDao.getUserProficiencies(username, abilityIdent);
			}
		} catch (Exception e) {
			e.printStackTrace();
			return null;
